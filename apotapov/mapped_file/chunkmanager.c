...
int ch_init(off_t index, off_t length, ch_pool_t *ch_pool) {
   ...
   // write_log_to_file(Info, "ch_init: started!\n");
    chunk_t *buf;
    if(ch_pool -> list_of_free_chunks -> size == 0) {
      //  write_log_to_file(Info, "ch_init: list of free chunks is empty!\n");
        if(ch_pool -> list_zero_ref_count -> size == 0) {
           // write_log_to_file(Info, "ch_init: list of chunks with 0 reference counter is empty!\n");
            ...
            for(i = 0; i < DEFAULT_ARRAY_SIZE; i++) {
                  list_add_last(ch_pool -> list_of_free_chunks, &((ch_pool -> pool)[ch_pool -> arrays_cnt - 1][i]));
                  (ch_pool -> pool)[ch_pool -> arrays_cnt - 1][i].data = NULL;
                  (ch_pool -> pool)[ch_pool -> arrays_cnt - 1][i].ref_counter = 0;
                  (ch_pool -> pool)[ch_pool -> arrays_cnt - 1][i].ch_pool = ch_pool;
                  (ch_pool -> pool)[ch_pool -> arrays_cnt - 1][i].index = 0;
                  (ch_pool -> pool)[ch_pool -> arrays_cnt - 1][i].length = 0;
            }
            buf = ch_pool -> list_of_free_chunks -> first -> data;
            buf -> ref_counter += 1;
            list_erase_first(ch_pool -> list_of_free_chunks);
        } else {
            buf = ch_pool -> list_zero_ref_count -> first -> data;
            buf -> ref_counter += 1;
            list_erase_first(ch_pool -> list_zero_ref_count);
   ...
   // write_log_to_file(Info, "ch_init: finished!\n");
    return 0;
}

int chunk_release(chunk_t* chunk) {
    // write_log_to_file(Info, "chunk_release: started!\n");
    if(chunk == NULL) {
    ...
    chunk -> length = 0;
    chunk -> data = NULL;
   // write_log_to_file(Info, "ch_release: finished!\n");
    return 0;
}

int chunk_clear(chunk_t* chunk) {
    //write_log_to_file(Info, "chunk_clear: started!\n");
    if(chunk == NULL) {
    ...
    chunk -> ch_pool = NULL;
   // write_log_to_file(Info, "chunk_clear: finished!\n");
    return 0;
}
...
ch_pool_t *ch_pool_init(int fd, int prot) {
   ...
   // write_log_to_file(Info, "ch_pool_init: started!\n");
   ...
   // write_log_to_file (Info, "ch_pool_init: finished!\n");
    return buf;
}

int ch_pool_deinit(ch_pool_t* ch_pool) {
   // write_log_to_file (Info, "ch_pool_deinit: started!\n");
    if(ch_pool == NULL) {
