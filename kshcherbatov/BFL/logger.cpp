...
#define DOUT(fmt, ... ) do {fprintf (stdout, fmt, ##__VA_ARGS__);} while (0)
#else
#define DOUT(fmt, ... )
#endif
...
static void ring_buff_destruct_shared_resources(struct ring_buff_t *ring_buff);
...
static inline bool ring_buff_is_ok(const struct ring_buff_t *ring_buff);
...
struct ring_buff_t *ring_buff_construct(const char log_filename[]) {
    ...
    if ((ring_buff->logfile_fd = open(log_filename, O_WRONLY|O_CREAT|O_ASYNC|O_APPEND, 0666)) < 0) {
        free((void *)ring_buff);
...
static inline void ring_buff_validate_daemon(struct ring_buff_t *ring_buff) {
    if (sem_act(ring_buff->sems_fd, SEM_CONTROL_HAS_DAEMON, SEM_ACTION_WAIT0, IPC_NOWAIT) == 0) {
        ...
        ring_buf_push_msg(ring_buff, LOGLEVEL_INFO, getpid(), "Daemon was dead. Created new daemon.\n");
    }
}
...
static void ring_buff_flush_to_file(struct ring_buff_t *ring_buff) {
    assert(ring_buff);
    assert(ring_buff_is_ok(ring_buff));
    ...
    while (shared->msg_buff[msg_buff_index].is_ready_to_flush) {
        ...
        const struct tm time_stamp = shared->msg_buff[msg_buff_index].time_stamp;

        char msg_to_write[LOG_MSG_MAX_SIZE + 26];

        int msg_to_write_size = snprintf(msg_to_write, LOG_MSG_MAX_SIZE + 15, "%d{%02d.%02d.%02d %02d:%02d:%02d}[%c] : %s",
                                         pid,
                                         time_stamp.tm_mday, time_stamp.tm_mon + 1, (time_stamp.tm_year + 1900) % 100,
                                         time_stamp.tm_hour, time_stamp.tm_min, time_stamp.tm_sec,
                                         loglevel_lbl[msg_log_level],
                                         msg);

        if (write(logfile_fd, msg_to_write, (size_t)msg_to_write_size) != msg_to_write_size)
...
void ring_buf_push_msg(struct ring_buff_t *ring_buff, enum LOGLEVEL_TYPES log_level, const pid_t pid, const char *fmt, ...) {
    ...
    struct tm time_stamp = *localtime(&time_info);
    shared->msg_buff[msg_buff_used_local].time_stamp = time_stamp;
...
void __init_log_system(int argc, char * argv[]) {
    ...
    int ch;
    if ((ch = getopt(argc, argv, "L:")) != -1 && ch == 'L') {
        Logging_system = ring_buff_construct(optarg);
    } else {
        fprintf(stderr, "Used standart log filename \'%s\'.\n", DEFAULT_LOG_FILENAME);
        Logging_system = ring_buff_construct(DEFAULT_LOG_FILENAME);
    }

    ring_buf_push_msg(Logging_system, LOGLEVEL_INFO, getpid(), "\n\n");
