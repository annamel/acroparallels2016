...
#define handle_error(msg) \
    do { perror(msg); deinit_log_system(); exit(EXIT_FAILURE); } while (0)
...
struct ring_buff_t *ring_buff_construct(const char log_filename[]) {
    ...
    if ((ring_buff->logfile_fd = open(log_filename, O_WRONLY|O_ASYNC|O_CREAT|O_APPEND, 0666)) < 0) {
        free((void *)ring_buff);
...
void init_log_system(const char log_filename[]) {
    ...
    if (!Logging_system)
        handle_error("Failed init Log system");
