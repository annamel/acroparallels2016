...
static int cpool_add(chunk_t *ch) {
	...
	for (unsigned i = 0; i < ch->len; ++i) {
		int err = hashtable_add(cpool->ht, ch->idx+i, (hval_t)ch);
		if(err) return err;
	}
...
int ch_acquire(cpool_t *cpool, off_t offset, size_t size, chunk_t **ch) {
	...
	off_t idx = offset / get_ch_size(1);
	off_t len = size / get_ch_size(1) + 1;
...
int ch_find(cpool_t *cpool, off_t offset, size_t size, chunk_t **ch) {
	...
	return ch_get(cpool, idx, len, ch);
}
...
int ch_get_mem(chunk_t *ch, off_t offset, void **buf) {
	...
	off_t left = get_ch_size(ch->idx);
	off_t right = left + get_ch_size(ch->len);

	if (offset < left || offset > right)
		return EINVAL;

	off_t coff = offset - left;
	*buf = ch->payload + coff;
...
int cpool_mem_add(void *ptr, chunk_t *ch) {
	return hashtable_add(ch->cpool->mem_ht, (hkey_t)ptr, (hval_t)ch);
}

int cpool_mem_get(cpool_t *cpool, void *ptr, chunk_t **ch) {
	ch = hashtable_get(cpool->mem_ht, (hkey_t)ptr);
	return 0;
}
