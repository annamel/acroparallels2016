...
#include <stdlib.h>
#include <string.h>
// @jjeka: #include <stdbool.h> и использовать bool, true, false?
...
int buffer_push(mutable_buffer_t* buffer, char c) // @jjeka: круто, нет ограничений на размер
{
...
void print_usage(const char* executable) // @jjeka клевый usage
{
	printf("\n");
	...
	printf("Replaces only whole numbers from input\n"
		   ...
		   "\tmoreover, no sign sign character precedes a cut number\n" // @jjeka sign написан дважды
		   "\n"
...
{	
	...
	int last_digit = 0;
	int reading_signs = 0;
	int reading_fraction = 0;
	
	mutable_buffer_t delayed_output;
	if (buffer_init(&delayed_output))
		ERROR("buffer_init: Insufficient memory");

	while (1) // @jjeka мне кажется, что использовать for (;;) более логично (хотя тут всегда споры)
	{
		int c = fgetc(input);
		...
		{			
			...
			reading_fraction = c == '.'; // @jjeka для лучшей читаемости в скобки запихнуть
...
int main(int argc, char** argv)
{
	...
	bizzbuzz(input ? input : stdin); // @jjeka: клевое решение
